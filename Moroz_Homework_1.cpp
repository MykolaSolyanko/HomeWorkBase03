...
int main() {
  ...
  while (attemptsCounter < numberOfAttempts) {
    ...
    if ((a >= minValue) && (a < std::numeric_limits<uint16_t>::max())) {
      if ((b >= minValue) && (a < std::numeric_limits<uint16_t>::max())) {
        if ((c >= minValue) && (a < std::numeric_limits<uint16_t>::max())) {
  ...
  if (attemptsCounter < numberOfAttempts) {
    ...
    if (discriminant > 0) {
      auto x1 = (-b + sqrt(discriminant)) / divider;
      auto x2 = (-b - sqrt(discriminant)) / divider;
