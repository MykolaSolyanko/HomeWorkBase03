...
void fill_arr(unsigned int* arr, int arr_bgn, int arr_end) {
    std::random_device rd;
    std::mt19937 mt(rd());
    std::uniform_int_distribution<unsigned int> rd_rng(
        std::numeric_limits<unsigned int>::min(),
        std::numeric_limits<unsigned int>::max());
    for (int i = arr_bgn; i < arr_end; ++i) {
        arr[i] = rd_rng(mt);
    }
}

void print_arr(unsigned int* arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        std::cout << arr[i] << " ";
    }
    std::cout << std::endl;
}
void print_arr(double* arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        std::cout << arr[i] << " ";
    }
    std::cout << std::endl;
}
void print_arr(char* arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        std::cout << arr[i] << " ";
    }
    std::cout << std::endl;
...
void bubble_sort(unsigned int* arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        for (int j = bgn; j < end - 1; ++j) {
            if (arr[j] > arr[j + 1]) {
                swap(&arr[j], &arr[j + 1]);
            }
        }
...
void bubble_sort(double* arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        for (int j = bgn; j < end - 1; ++j) {
            if (arr[j] > arr[j + 1]) {
                swap(&arr[j], &arr[j + 1]);
...
int partition(unsigned int* arr, int bgn, int end) {
...
void quick_sort(unsigned int* arr, int bgn, int end) {
    if (bgn < end) {
...
int partition(char* arr, int bgn, int end) {
    char pivot = arr[end];
    int i = (bgn - 1);
...
void quick_sort(char* arr, int bgn, int end) {
    if (bgn < end) {
        int p = partition(arr, bgn, end);
        quick_sort(arr, bgn, p - 1);
        quick_sort(arr, p + 1, end);
...
int partition(double* arr, int bgn, int end) {
...
void quick_sort(double* arr, int bgn, int end) {
    if (bgn < end) {
        int p = partition(arr, bgn, end);
        quick_sort(arr, bgn, p - 1);
        quick_sort(arr, p + 1, end);
...
void locate_dgts_at_end(char* arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        for (int j = bgn; j < end - 1; ++j) {
...
void bubble_sort(char* arr, int bgn, int end) {
    int lst_dgt_pos{ 0 };
    for (int i = bgn; i < end; ++i) {
        for (int j = bgn; j < end - 1; ++j) {
            if (arr[j] > arr[j + 1]) {
...
void cast_arr(unsigned int* f_arr, double* t_arr, int bgn, int end) {
    double magic_num_for_cast = 5;
    for (int i = bgn; i < end; ++i) {
        t_arr[i] = f_arr[i] / magic_num_for_cast;
    }
}
void cast_arr(char* charr, int bgn, int end) {
    std::random_device rd;
    std::mt19937 mt(rd());
...
void measure_bubble(unsigned int* uiarr, double* darr, char* charr, int bgn,
    int end) {
...
void measure_quick(unsigned int* uiarr, double* darr, char* charr, int bgn,
    int end) {
    auto uistart_t = std::chrono::system_clock::now();
...
void prepare_data(unsigned int* uiarr, char* charr, double* darr, int bgn,
    int end) {
...
void copy_arr(unsigned int* f_arr, unsigned int* t_arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        t_arr[i] = f_arr[i];
    }
}

void copy_arr(double* f_arr, double* t_arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        t_arr[i] = f_arr[i];
    }
}

void copy_arr(char* f_arr, char* t_arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        t_arr[i] = f_arr[i];
    }
}

bool compare_arr(unsigned int* b_arr, unsigned int* q_arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        if (b_arr[i] != q_arr[i]) {
            return false;
        }
...
bool compare_arr(char* b_arr, char* q_arr, int bgn, int end) {
    for (int i = bgn; i < end; ++i) {
        if (b_arr[i] != q_arr[i]) {
            return false;
        }
...
int main() {
	// при вызове quic_sort падает в runtime с ошибкой 
	// stack around the variable 'uiarr_qui' was corrupted
    ...
    // std::endl; std::cout << compare_arr(charr_bub, charr_qui, strt_arr, arr_len)
    // << std::endl;
    return 0;
}