...
void copy_arr(unsigned int* f_arr, unsigned int* t_arr, size_t bgn, size_t end) {
    for (size_t i = bgn; i < end; ++i) {
        t_arr[i] = f_arr[i];
    }
}
...
bool compare_arr(unsigned int* b_arr, unsigned int* q_arr, size_t bgn, size_t end) {
    for (size_t i = bgn; i < end; ++i) {
        if (b_arr[i] != q_arr[i]) {
            return false;
        }
